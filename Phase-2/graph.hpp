...
public:
    ...
    void dfs_fill_order(int u, std::vector<bool>& visited, std::vector<int>& stack);
    void dfs_assign_component(int u, int c, std::vector<bool>& visited);
    double run_dijkstra_fallback(int source, int target);
    void run_dijkstra_core(int src, const std::vector<std::vector<Edge*>>& adjacency, std::vector<double>& dists);
    void precompute_alt();
